...
- **Step 12.5 (structured twin):** render the SAME synthesized data to `trips/{slug}/dossier-data.json` per the template's § Structured data companion. Stable `id` per place; `image_query` + empty `image_url` slot; never fabricate coordinates or image URLs; never include a value the markdown doesn't support.
