...
- **Steps 3, 5, 7, 9 (prompt generation per pass):** substitute placeholders from `subagent-prompts.md`. Branch Step 3 on `trip_type` (Prompt 2a for single-base, Prompt 2b — candidate places, NOT a finished route — for multi-stop). Do not improvise prompt bodies. Step 9: do NOT generate Prompt 5c (timing) — timing was collected at `/trip-init` time and is read from `trip-context.md` frontmatter fields `timing_verdict` / `timing_notes`. Step 9: 5d is now **Practical logistics** (connectivity / offline maps / entry-admin / borders), not music. Apply the **short-stop skip rule** for 5a — scope to stops with 3+ nights; if every stop is ≤ 2 nights, skip 5a entirely.
- **Step 4 (place-selection gate):** this is a DISCUSSION. Present Pass 2, engage with Patrik's questions/comparisons, and converge on the set of places together. **Never pick the destinations or the route for him.** Hard rule: do not proceed to Step 4.4 / Pass 3 until he has explicitly approved the place set; if ambiguous, ask.
- **Step 4.4 (route + nights, multi-stop only):** after the place set is approved, propose an order + nights split as a confirmable proposal; Patrik confirms or adjusts before it locks. No subagent — orchestrator↔operator exchange. Skip for single-base.
- **Steps 6, 8, 10 (pauses):** present each pass prompt labeled with pass number, target tool, and expected output shape. End with the paste-back and KEEP format instructions. Stop and wait. Do not generate dossier content until Patrik responds. Step 10: present 5a (if run), 5b, 5d — omit 5c.
- **Steps 4.5, 6.5, 8.5, 10.5 (ingest):** parse KEEP selections against long lists. Fuzzy-match names as fallback; ask for clarification on ambiguous matches. Write `pass-N-state.md` for each completed pass. For Pass 2, also update `trip-context.md` `approved_locations` and `route_stops` (from the Step 4.4 confirmed route) via Edit. At Step 10.5, after parsing the 5a short list, run the de-duplication check: read `pass-3-state.md` and `pass-4-state.md` short lists; fuzzy-match 5a items against them; remove conflicts from Pass 3/4 short lists (trap wins); log each conflict. If more than 3 conflicts, surface to operator before proceeding.
- **Step 11 (synthesize):** apply anti-tourist filter, personalization filter, routine integration check. Assign one priority tier (🔥 HIGH / 👍 REC / 🆗 OK) to each Section 3 and 4 place and pick exactly one ⭐ anchor per stop. Record what you filtered out and the tier/anchor decisions — this data goes to the run log, not the dossier (per Decision 7 in `pipeline/decisions.md`).
- **Step 12 (format):** follow `dossier-template.md`. Sections 3–4 use per-place cards sorted by tier with the per-stop anchor line; omit Section 5 for ≤2-night stops; Section 8 is Practical logistics. Apply multi-stop section variants for Sections 2 and 6 when `trip_type` is `multi-stop`. NO grounding evidence block in the output.
- **Step 12.5 (structured twin):** render the SAME synthesized data to `trips/{slug}/dossier-data.json` per the template's § Structured data companion. The JSON matches the HTML renderer's model exactly (`trip` array of stops + `legs` array). Use the controlled vocabulary only (integer tier `t` 3/2/1, the fixed tag list, the fixed `facts` icon ids); compose `facts` from the gathered fields; map anchor→`start`, inter-stop legs→`legs`, logistics→per-stop `connectivity`/`borders` + `meta.admin`. Omit `avoid` for ≤2-night stops. Stable `id` per stop/place; `image_query` + empty `image_url`; never fabricate coordinates or image URLs; never include a value the markdown doesn't support. Do NOT generate or modify the HTML shell — Patrik owns it.
- **Step 13 (write dossier):** write to `trips/{slug}/destination-dossier.md` (or `-v{n}.md` if a prior version exists) and the matching `dossier-data.json`. Do not overwrite.
