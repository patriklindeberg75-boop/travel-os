...
- **Destination intro + temp (§1).** Author a short `intro` per stop (what it's like, why included, what it's good for, how to approach it) and a plain `temp` string (no commentary words). Omit `temp` if no real forecast is known.
